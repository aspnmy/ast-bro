...
- **`src/mcp/tools.rs`**: declares seventeen tool schemas (`map`, `digest`, `show`, `implements`, `callers`, `callees`, `trace`, `surface`, `squeeze`, `deps`, `reverse_deps`, `cycles`, `graph`, `search`, `find_related`, `index`, `run`) and dispatches `tools/call` into the existing `core::render_*` / `calls::*` / `surface::*` / `deps::*` / `search::*` functions. Each tool maps 1:1 to a CLI subcommand and reuses its render logic byte-for-byte, so the JSON schemas are shared with the CLI's `--json` output.
