...
release:
	cargo build --release
	zip -j target/release/ast-outline-macos-arm64.zip \
...
## Update Formula SHA256 from local release zip (run after release-macos, before upload).
## On the ast-outline branch the release zip is named ast-outline-macos-arm64.zip
## and both formulas reference it.
##   make update-formula
update-formula:
